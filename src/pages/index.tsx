        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
          <div className="bg-white rounded border shadow-sm overflow-hidden">
            ...
            <table className="w-full text-xs border-collapse">
              <thead><tr className="bg-gray-100"><th className="px-2 py-1.5 text-left">Project</th><th className="px-2 py-1.5 text-right">Pending</th><th className="px-2 py-1.5 text-right">Sent</th></tr></thead>
          ...
          <div className="bg-white rounded border shadow-sm overflow-hidden">
            ...
            <div className="overflow-auto">
              <table className="w-full text-xs border-collapse">
                <thead><tr className="bg-gray-100"><th className="px-2 py-1.5 text-left">Project</th><th className="px-2 py-1.5 text-right">Sales (T)</th><th className="px-2 py-1.5 text-right">Coll. (T)</th><th className="px-2 py-1.5 text-right">Coll. (M)</th><th className="px-2 py-1.5 text-right">Till Date</th></tr></thead>
                <tbody>{(dashData as any[]).map((d: any, i) => (
                  <tr key={i} className={i%2===0?'bg-white':'bg-gray-50'}>
                    <td className="px-2 py-1.5 font-medium">{d.project.name}</td>
          ...
          <div className="bg-white rounded border shadow-sm overflow-hidden">
            ...
            <table className="w-full text-xs border-collapse">
              <thead><tr className="bg-gray-100"><th className="px-2 py-1.5 text-left">Project</th><th className="px-2 py-1.5 text-right">Total</th><th className="px-2 py-1.5 text-right">Available</th><th className="px-2 py-1.5 text-right">Booked</th><th className="px-2 py-1.5 text-right">Sold</th></tr></thead>
              <tbody>{(dashData as any[]).map((d: any, i) => (
                <tr key={i} className={i%2===0?'bg-white':'bg-gray-50'}>
                  <td className="px-2 py-1.5 font-medium">{d.project.name}</td>
      ...
      <div className="bg-slate-800 text-white rounded p-2 flex gap-2 flex-wrap">
        {quickActions.map(a => (
          <Link key={a.label} href={a.href}>
            <button className="bg-slate-700 hover:bg-orange-600 transition-colors text-xs px-3 py-1.5 rounded">{a.label}</button>
          </Link>
        ))}
      </div>
    </div>
