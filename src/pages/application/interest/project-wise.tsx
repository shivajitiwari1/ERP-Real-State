...
export default function InterestProjectWisePage() {
  ...
  return (
    <div>
      <PageHeader title="Interest Master (Project Wise)" />
      <div className="space-y-4">
        ...
        <div className="bg-white border rounded-lg shadow-sm overflow-hidden">
          <div className="bg-purple-700 px-3 py-2 text-white text-xs font-bold uppercase">Interest Master (Project Wise) ({withInterest.length} overdue)</div>
          <div className="overflow-auto">
            <table className="w-full text-xs">
              <thead><tr className="bg-slate-800 text-white"><th className="px-3 py-2">S.No.</th><th className="px-3 py-2">Reg. No.</th><th className="px-3 py-2">Project</th><th className="px-3 py-2">Principal (₹)</th><th className="px-3 py-2">Due Date</th><th className="px-3 py-2">Days Overdue</th><th className="px-3 py-2">Interest (₹)</th></tr></thead>
              <tbody>
                ...
                withInterest.map((d: any, i: number) => (
                  <tr key={d.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-3 py-2 text-gray-600">{d.Booking?.Project?.name || '—'}</td>
