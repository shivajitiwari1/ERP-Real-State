...
export default function GraceProjectWisePage() {
  ...
  const withGrace = (demands as any[]).filter((d: any) => d.dueDate).map((d: any) => {
    ...
    return { ...d, daysOverdue: days, effectiveDays, interest: Math.round(interest), status: d.status };
  }).filter((d: any) => d.daysOverdue > 0 && d.status !== 'settled');

  return (
    <div>
      <PageHeader title="Project Wise Grace Period" />
