...
export default function InterestCustomerDeletePage() {
  ...
  const overdue = (demands as any[]).filter((d: any) => d.dueDate && new Date(d.dueDate) < new Date() && d.status !== 'settled' && !deleted.has(String(d.id)));
  ...
  return (
    <div>
      <PageHeader title="Delete Customer Interest" />
      <div className="space-y-4">
        ...
        <div className="bg-white border rounded-lg shadow-sm overflow-hidden">
          <div className="bg-purple-700 px-3 py-2 text-white text-xs font-bold uppercase">Delete Customer Interest ({withInterest.length} overdue)</div>
          <div className="overflow-auto">
            <table className="w-full text-xs">
              <thead><tr className="bg-slate-800 text-white"><th className="px-3 py-2">S.No.</th><th className="px-3 py-2">Reg. No.</th><th className="px-3 py-2">Principal (₹)</th><th className="px-3 py-2">Due Date</th><th className="px-3 py-2">Days Overdue</th><th className="px-3 py-2">Interest (₹)</th><th className="px-3 py-2">Action</th></tr></thead>
              <tbody>
                ...
                withInterest.map((d: any, i: number) => (
                  <tr key={d.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-3 py-2"><button onClick={() => setDeleted(prev => new Set(Array.from(prev).concat(String(d.id))))} className="px-2 py-1 bg-red-600 text-white rounded text-xs hover:bg-red-700">Delete</button></td>
                  </tr>
                ))}
              </tbody>
