...
export default function InterestWaiveOffPage() {
  ...
  const withInterest = overdue.map((d: any) => {
    ...
    return { ...d, daysOverdue: days, interest: Math.round(interest), isWaived: waived.has(String(d.id)) };
  });

  return (
    <div>
      <PageHeader title="Interest Waive Off" />
      <div className="space-y-4">
        ...
        <div className="bg-white border rounded-lg shadow-sm overflow-hidden">
          <div className="bg-purple-700 px-3 py-2 text-white text-xs font-bold uppercase">Interest Waive Off ({withInterest.length} overdue)</div>
              ...
              <tbody>
                ...
                withInterest.map((d: any, i: number) => (
                  <tr key={d.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-3 py-2 text-orange-600 font-semibold">{d.isWaived ? <span className="line-through text-gray-400">₹{d.interest.toLocaleString('en-IN')}</span> : `₹${d.interest.toLocaleString('en-IN')}`}</td>
                    <td className="px-3 py-2">{d.isWaived ? <span className="text-green-600 font-medium">Waived</span> : <span className="text-orange-500">Pending</span>}</td>
                    <td className="px-3 py-2">{!d.isWaived && <button onClick={() => setWaived(prev => new Set(Array.from(prev).concat(String(d.id))))} className="px-2 py-1 bg-green-700 text-white rounded text-xs hover:bg-green-800">Waive Off</button>}</td>
                  </tr>
                ))}
              </tbody>
              {withInterest.length > 0 && <tfoot><tr className="bg-slate-100 font-bold"><td colSpan={2} className="px-3 py-2">Total</td><td className="px-3 py-2">₹{withInterest.reduce((s: number, d: any) => s + Number(d.totalAmount), 0).toLocaleString('en-IN')}</td><td colSpan={2}></td><td className="px-3 py-2 text-orange-600">₹{withInterest.filter((d: any) => !d.isWaived).reduce((s: number, d: any) => s + d.interest, 0).toLocaleString('en-IN')}</td><td colSpan={2}></td></tr></tfoot>}
            </table>
          </div>
        </div>
      </div>
