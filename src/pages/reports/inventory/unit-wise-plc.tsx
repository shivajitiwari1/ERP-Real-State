...
export default function UnitWisePLCPage() {
  const [projectId, setProjectId] = useState('');
  const { data: projects = [] } = useQuery<any[]>({ queryKey: ['projects'], queryFn: () => axios.get('/api/projects').then(r => r.data.data) });
  const { data: bookings = [], isLoading } = useQuery<any[]>({ queryKey: ['inv-plc', projectId], queryFn: () => axios.get(`/api/application/bookings${projectId ? `?projectId=${projectId}` : ''}`).then(r => r.data.data) });

  return (
    <div>
      <PageHeader title="Unit Wise PLC" />
      <div className="space-y-4">
        ...
        <div className="bg-white rounded border shadow-sm overflow-hidden">
          <div className="bg-purple-700 text-white text-xs font-bold px-3 py-2 uppercase tracking-wide">Unit Wise PLC ({(bookings as any[]).length})</div>
          <table className="w-full text-xs border-collapse">
            <thead><tr className="bg-slate-100">
              ...
              <th className="px-3 py-2 text-right">PLC Amount (₹)</th>
              <th className="px-3 py-2 text-left">PLC Type</th>
            ...
            <tbody>
              ...
              (bookings as any[]).map((b: any, i: number) => (
                <tr key={b.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-3 py-1.5">{b.Unit?.floor || '-'}</td>
                  <td className="px-3 py-1.5">{b.Unit?.unitType || '-'}</td>
                  <td className="px-3 py-1.5 text-right text-purple-600">{b.Unit?.area || '-'}</td>
                  <td className="px-3 py-1.5 text-right text-green-600">{Number(b.Unit?.baseRate || 0).toLocaleString('en-IN')}</td>
                  <td className="px-3 py-1.5 text-right text-blue-600">{Number(b.plcAmount || b.Unit?.plcAmount || 0).toLocaleString('en-IN')}</td>
                  <td className="px-3 py-1.5">{b.plcType || 'Standard'}</td>
