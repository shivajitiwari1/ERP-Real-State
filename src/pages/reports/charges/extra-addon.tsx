...
export default function ExtraAddonReportPage() {
  const [projectId, setProjectId] = useState('');
  const { data: projects = [] } = useQuery<any[]>({ queryKey: ['projects'], queryFn: () => axios.get('/api/projects').then(r => r.data.data) });
  const { data: receipts = [], isLoading } = useQuery<any[]>({ queryKey: ['extra-addon', projectId], queryFn: () => axios.get(`/api/application/receipts/index${projectId ? `?projectId=${projectId}` : ''}`).then(r => r.data.data) });

  const addonReceipts = (receipts as any[]).filter((r: any) => r.receiptType === 'addon' || r.chargeType === 'addon');
  const grandTotal = addonReceipts.reduce((s, r: any) => s + Number(r.totalAmount || r.amount || 0), 0);

  return (
    <div>
      <PageHeader title="Extra Addon Report" />
        ...
        <div className="bg-white rounded border shadow-sm overflow-hidden">
          ...
          <table className="w-full text-xs border-collapse">
            <thead><tr className="bg-slate-100">
              ...
              <th className="px-3 py-2 text-left">Addon Type</th>
              <th className="px-3 py-2 text-right">Amount (₹)</th>
            </tr></thead>
            <tbody>
              {isLoading ? <tr><td colSpan={7} className="text-center py-6 text-gray-400">Loading...</td></tr> :
              addonReceipts.length === 0 ? <tr><td colSpan={7} className="text-center py-6 text-gray-400">No addon receipts found</td></tr> :
              addonReceipts.map((r: any, i: number) => (
                <tr key={r.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-3 py-1.5 text-purple-600">{r.addonType || r.chargeDescription || 'addon'}</td>
