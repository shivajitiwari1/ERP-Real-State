...
export default function TDSReportPage() {
  ...
  const { data: receipts = [], isLoading } = useQuery<any[]>({ queryKey: ['stax-tds', projectId], queryFn: () => axios.get(`/api/application/receipts/index${projectId ? `?projectId=${projectId}` : ''}`).then(r => r.data.data) });

  return (
    <div>
      <PageHeader title="Customer Wise TDS Report" />
      <div className="space-y-4">
        <div className="bg-white border rounded-lg shadow-sm p-3 flex gap-3 items-center">
          ...
          <span className="text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200 rounded-md px-3 py-1.5">TDS Rate: 1%</span>
        </div>
        <div className="bg-white rounded border shadow-sm overflow-hidden">
          <div className="bg-purple-700 text-white text-xs font-bold px-3 py-2 uppercase tracking-wide">Customer Wise TDS Report ({(receipts as any[]).length})</div>
          <table className="w-full text-xs border-collapse">
            <thead><tr className="bg-slate-100">
              ...
              <th className="px-3 py-2 text-right">TDS 1% (₹)</th>
            ...
            <tbody>
              ...
              (receipts as any[]).map((r: any, i: number) => {
                const amt = Number(r.totalAmount || r.amount || 0); const tds = amt * TDS_RATE;
                return (
                  <tr key={r.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-3 py-1.5 text-right text-orange-600">{tds.toLocaleString('en-IN')}</td>
                    <td className="px-3 py-1.5 text-right text-purple-600 font-bold">{(amt - tds).toLocaleString('en-IN')}</td>
                  </tr>
                );
              })}
            </tbody>
