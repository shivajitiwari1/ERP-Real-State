...
export default function DuesPlanWiseTaxPage() {
  const [projectId, setProjectId] = useState('');
  const { data: projects = [] } = useQuery<any[]>({ queryKey: ['projects'], queryFn: () => axios.get('/api/projects').then(r => r.data.data) });
  const { data: demands = [], isLoading } = useQuery<any[]>({ queryKey: ['stax-dues-plan-wise', projectId], queryFn: () => axios.get(`/api/application/demand/list${projectId ? `?projectId=${projectId}` : ''}`).then(r => r.data.data) });

  const grouped: Record<string, { count: number; total: number }> = {};
  (demands as any[]).forEach((d: any) => {
    const plan = d.demandType || d.planType || 'Unknown';
    if (!grouped[plan]) grouped[plan] = { count: 0, total: 0 };
    grouped[plan].count++;
    grouped[plan].total += Number(d.totalAmount || d.amount || 0);
  });
  const rows = Object.entries(grouped).map(([plan, d]) => ({ plan, ...d, gst: d.total * GST }));

  return (
    <div>
      <PageHeader title="Plan Wise Due (Tax)" />
      <div className="space-y-4">
        ...
        <div className="bg-white rounded border shadow-sm overflow-hidden">
          <div className="bg-purple-700 text-white text-xs font-bold px-3 py-2 uppercase tracking-wide">Plan Wise Due (Tax) ({rows.length})</div>
