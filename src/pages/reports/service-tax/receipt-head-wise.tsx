...
export default function ReceiptHeadWiseTaxPage() {
  const [projectId, setProjectId] = useState('');
  const { data: projects = [] } = useQuery<any[]>({ queryKey: ['projects'], queryFn: () => axios.get('/api/projects').then(r => r.data.data) });
  const { data: receipts = [], isLoading } = useQuery<any[]>({ queryKey: ['stax-receipt-head-wise', projectId], queryFn: () => axios.get(`/api/application/receipts/index${projectId ? `?projectId=${projectId}` : ''}`).then(r => r.data.data) });

  const grouped: Record<string, { count: number; total: number }> = {};
  (receipts as any[]).forEach((r: any) => {
    const head = r.receiptType || r.paymentHead || r.chargeType || 'General';
    if (!grouped[head]) grouped[head] = { count: 0, total: 0 };
    grouped[head].count++;
    grouped[head].total += Number(r.amount || 0);
  });
  const rows = Object.entries(grouped).map(([head, d]) => ({ head, ...d, gst: d.total * GST }));

  return (
    <div>
      <PageHeader title="Receipt Head Wise Service Tax" />
      <div className="space-y-4">
        ...
        <div className="bg-white rounded border shadow-sm overflow-hidden">
          <div className="bg-purple-700 text-white text-xs font-bold px-3 py-2 uppercase tracking-wide">Receipt Head Wise Service Tax ({rows.length})</div>
          <table className="w-full text-xs border-collapse">
            ...
            <tbody>
              ...
              rows.map((r, i) => (
                <tr key={r.head} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
