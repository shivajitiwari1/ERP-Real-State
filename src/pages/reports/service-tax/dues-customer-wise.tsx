...
export default function DuesCustomerWiseTaxPage() {
  const [projectId, setProjectId] = useState('');
  const { data: projects = [] } = useQuery<any[]>({ queryKey: ['projects'], queryFn: () => axios.get('/api/projects').then(r => r.data.data) });
  const { data: demands = [], isLoading } = useQuery<any[]>({ queryKey: ['stax-dues-customer-wise', projectId], queryFn: () => axios.get(`/api/application/demand/list${projectId ? `?projectId=${projectId}` : ''}`).then(r => r.data.data) });

  const grouped: Record<string, { name: string; unit: string; total: number; pending: number }> = {};
  (demands as any[]).forEach((d: any) => {
    ...
    if (!grouped[key]) grouped[key] = { name, unit: d.Booking?.Unit?.unitNumber || '-', total: 0, pending: 0 };
    grouped[key].total += Number(d.totalAmount || d.amount || 0);
    grouped[key].pending += Number(d.pendingAmount || 0);
  });
  const rows = Object.entries(grouped).map(([key, d]) => ({ key, ...d, gst: d.total * GST }));

  return (
    <div>
      <PageHeader title="Customer Wise Due (Tax)" />
      <div className="space-y-4">
        ...
        <div className="bg-white rounded border shadow-sm overflow-hidden">
          <div className="bg-purple-700 text-white text-xs font-bold px-3 py-2 uppercase tracking-wide">Customer Wise Due (Tax) ({rows.length})</div>
            ...
            <tbody>
              ...
              rows.map((r, i) => (
                <tr key={r.key} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-3 py-1.5 text-right text-red-600">{r.pending.toLocaleString('en-IN')}</td>
                </tr>
              ))}
            </tbody>
          </table>
