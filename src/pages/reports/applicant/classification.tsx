...
export default function CustomerClassificationPage() {
  const [projectId, setProjectId] = useState('');
  const { data: projects = [] } = useQuery<any[]>({ queryKey: ['projects'], queryFn: () => axios.get('/api/projects').then(r => r.data.data) });
  const { data: bookings = [], isLoading } = useQuery<any[]>({ queryKey: ['appl-classification', projectId], queryFn: () => axios.get(`/api/application/bookings${projectId ? `?projectId=${projectId}` : ''}`).then(r => r.data.data) });

  const grouped: Record<string, { count: number; totalValue: number }> = {};
  (bookings as any[]).forEach((b: any) => {
    const type = b.Applicants?.[0]?.applicantType || b.customerType || 'General';
    if (!grouped[type]) grouped[type] = { count: 0, totalValue: 0 };
    grouped[type].count++;
    grouped[type].totalValue += Number(b.agreementValue || b.totalAmount || 0);
  });
  const rows = Object.entries(grouped).map(([type, d]) => ({ type, ...d }));
  const totalCount = (bookings as any[]).length;

  return (
    <div>
      <PageHeader title="Customer Classification" />
      <div className="space-y-4">
        ...
        <div className="bg-white rounded border shadow-sm overflow-hidden">
          <div className="bg-purple-700 text-white text-xs font-bold px-3 py-2 uppercase tracking-wide">Customer Classification ({rows.length})</div>
            ...
            <tbody>
              ...
              rows.map((r, i) => (
                <tr key={r.type} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-3 py-1.5 text-right">{totalCount > 0 ? `${((r.count / totalCount) * 100).toFixed(1)}%` : '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
