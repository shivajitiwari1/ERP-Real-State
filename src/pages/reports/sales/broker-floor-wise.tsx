...
export default function BrokerFloorWisePage() {
  const [projectId, setProjectId] = useState('');
  const { data: projects = [] } = useQuery<any[]>({ queryKey: ['projects'], queryFn: () => axios.get('/api/projects').then(r => r.data.data) });
  const { data: bookings = [], isLoading } = useQuery<any[]>({ queryKey: ['broker-floor-wise', projectId], queryFn: () => axios.get(`/api/application/bookings${projectId ? `?projectId=${projectId}` : ''}`).then(r => r.data.data) });

  const brokerBookings = (bookings as any[]).filter((b: any) => b.brokerId || b.Broker);

  return (
    <div>
      <PageHeader title="Broker Floor Wise Detail" />
      <div className="space-y-4">
        ...
        <div className="bg-white rounded border shadow-sm overflow-hidden">
          <div className="bg-purple-700 text-white text-xs font-bold px-3 py-2 uppercase tracking-wide">Broker Floor Wise Detail ({brokerBookings.length})</div>
            ...
            <tbody>
              ...
              brokerBookings.map((b: any, i: number) => (
                <tr key={b.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-3 py-1.5 text-purple-600">{b.Unit?.floor || '-'}</td>
