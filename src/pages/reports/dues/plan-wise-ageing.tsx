...
export default function PlanWiseAgeingPage() {
  const [projectId, setProjectId] = useState('');
  const { data: projects = [] } = useQuery<any[]>({ queryKey: ['projects'], queryFn: () => axios.get('/api/projects').then(r => r.data.data) });
  const { data: demands = [], isLoading } = useQuery<any[]>({ queryKey: ['plan-wise-ageing', projectId], queryFn: () => axios.get(`/api/application/demand/list${projectId ? `?projectId=${projectId}` : ''}`).then(r => r.data.data) });
  ...
  (demands as any[]).forEach((d: any) => {
    ...
    if (!grouped[plan]) grouped[plan] = { b0_30: 0, b31_60: 0, b61_90: 0, b90p: 0, total: 0 };
    const due = d.dueDate ? new Date(d.dueDate) : null;
    const pending = Number(d.pendingAmount || (d.totalAmount || d.amount || 0) - (d.paidAmount || 0));
    if (pending <= 0) return;
    grouped[plan].total += pending;
    if (!due) { grouped[plan].b0_30 += pending; return; }
    const days = Math.floor((today.getTime() - due.getTime()) / 86400000);
    if (days <= 30) grouped[plan].b0_30 += pending;
    else if (days <= 60) grouped[plan].b31_60 += pending;
    else if (days <= 90) grouped[plan].b61_90 += pending;
    else grouped[plan].b90p += pending;
  });
  const rows = Object.entries(grouped).map(([plan, d]) => ({ plan, ...d }));

  return (
    <div>
      <PageHeader title="Plan Wise Ageing" />
      <div className="space-y-4">
        ...
        <div className="bg-white rounded border shadow-sm overflow-hidden">
          <div className="bg-purple-700 text-white text-xs font-bold px-3 py-2 uppercase tracking-wide">Plan Wise Ageing ({rows.length})</div>
