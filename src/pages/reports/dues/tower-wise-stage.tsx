...
export default function TowerWiseStagePage() {
  const [projectId, setProjectId] = useState('');
  const { data: projects = [] } = useQuery<any[]>({ queryKey: ['projects'], queryFn: () => axios.get('/api/projects').then(r => r.data.data) });
  const { data: demands = [], isLoading } = useQuery<any[]>({ queryKey: ['dues-tower-stage', projectId], queryFn: () => axios.get(`/api/application/demand/list${projectId ? `?projectId=${projectId}` : ''}`).then(r => r.data.data) });
  ...
  (demands as any[]).forEach((d: any) => {
    ...
    if (!grouped[tower]) grouped[tower] = { count: 0, total: 0, paid: 0, pending: 0 };
    grouped[tower].count++;
    grouped[tower].total += Number(d.totalAmount || d.amount || 0);
    grouped[tower].paid += Number(d.paidAmount || 0);
    grouped[tower].pending += Number(d.pendingAmount || (d.totalAmount || d.amount || 0) - (d.paidAmount || 0));
  });
  ...
  return (
    <div>
      <PageHeader title="Tower Wise Dues" />
      <div className="space-y-4">
        ...
        <div className="bg-white rounded border shadow-sm overflow-hidden">
          <div className="bg-purple-700 text-white text-xs font-bold px-3 py-2 uppercase tracking-wide">Tower Wise Dues ({rows.length})</div>
