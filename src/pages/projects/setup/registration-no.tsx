...
type FormData = { registrationNoPrefix: string };

export default function RegistrationNoPage() {
  ...
  const { register, handleSubmit } = useForm<FormData>({
    values: cfg ? { registrationNoPrefix: cfg.registrationNoPrefix ?? 'REG' } : undefined,
  });
  ...
  return (
    <div>
      <PageHeader title="Registration No." />
        ...
        {projectId && !isLoading && (
          <div className="bg-white border rounded-lg shadow-sm p-5 max-w-md">
            <h3 className="text-xs font-bold text-slate-600 uppercase border-b pb-2 mb-4">
              Registration Number Prefix — {projName}
            </h3>
            <form onSubmit={handleSubmit(d => save.mutate(d))} className="space-y-4">
              <div>
                <Label className="text-xs">Registration No. Prefix</Label>
                <Input
                  {...register('registrationNoPrefix')}
                  placeholder="e.g. REG"
                  className="mt-1 h-9 text-sm font-mono uppercase"
                  maxLength={10}
                />
                <p className="text-xs text-gray-400 mt-1">
                  This prefix will be prepended to all registration numbers for this project.
                  Example: <strong className="font-mono">REG-0001</strong>
                </p>
              </div>
              {cfg?.registrationNoPrefix && (
                <div className="bg-blue-50 rounded p-3 text-xs text-blue-700">
                  Current prefix: <strong className="font-mono">{cfg.registrationNoPrefix}</strong>
                  &nbsp;→ e.g. <strong className="font-mono">{cfg.registrationNoPrefix}-0001</strong>
                </div>
              )}
