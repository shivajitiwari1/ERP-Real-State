...
type FormData = { transferAuthType: 'auto' | 'manual' };
...
export default function TransferAuthPage() {
  ...
  const { register, handleSubmit } = useForm<FormData>({
    values: cfg ? { transferAuthType: cfg.transferAuthType ?? 'auto' } : undefined,
  });
  ...
  return (
    <div>
      <PageHeader title="Transfer Authentication" />
            ...
            <form onSubmit={handleSubmit(d => save.mutate(d))} className="space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  <strong>Automated:</strong> System auto-approves unit transfer requests.
                  <br />
                  <strong>Manual:</strong> Admin must review and approve each transfer.
                </p>
              </div>
              {cfg && (
                <div className="bg-blue-50 rounded p-3 text-xs text-blue-700">
                  Current setting: <strong className="capitalize">{cfg.transferAuthType}</strong>
                </div>
              )}
