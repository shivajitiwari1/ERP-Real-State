...
export default function IFMSChargesPage() {
  ...
  const { data: charges = [] } = useQuery({ queryKey: ['ifms', selectedProject], queryFn: () => axios.get(`/api/projects/setup/ifms?projectId=${selectedProject}`).then(r => r.data.data), enabled: !!selectedProject });
  const save = useMutation({
    mutationFn: (d: FD) => d.id ? axios.put('/api/projects/setup/ifms', d) : axios.post('/api/projects/setup/ifms', d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['ifms'] }); reset({ rate: 0 }); },
  });
  const del = useMutation({ mutationFn: (id: number) => axios.delete('/api/projects/setup/ifms', { data: { id } }), onSuccess: () => qc.invalidateQueries({ queryKey: ['ifms'] }) });
  return (
    <div>
      <PageHeader title="IFMS Charges" subtitle="Interest Free Maintenance Security charges per project" />
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-white p-4 rounded border shadow-sm space-y-3">
          <form onSubmit={handleSubmit(d => save.mutate({ ...d, projectId: Number(d.projectId), rate: Number(d.rate) }))} className="space-y-3">
            ...
            <div><Label className="text-xs">IFMS Name *</Label><Input {...register('name')} placeholder="e.g. Maintenance Security" className="mt-1 h-9 text-sm" /></div>
        ...
        <div className="bg-white p-4 rounded border shadow-sm overflow-auto">
          <h3 className="text-xs font-bold text-slate-600 mb-3 pb-1 border-b uppercase">IFMS Charges ({(charges as any[]).length})</h3>
          <table className="w-full text-xs border-collapse">
            <thead><tr className="bg-slate-700 text-white">{['#','Name','Rate (Rs.)','Action'].map(h=><th key={h} className="px-2 py-2 text-left">{h}</th>)}</tr></thead>
