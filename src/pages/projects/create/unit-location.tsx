...
export default function UnitLocationPage() {
  ...
  const { data: records = [] } = useQuery<any[]>({
    queryKey: ['unit-locations', projectId],
    queryFn: () => axios.get(`/api/projects/unit-locations?projectId=${projectId}`).then(r => r.data.data),
    enabled: !!projectId,
  });
  ...
  const save = useMutation({
    mutationFn: (d: FD) => d.id ? axios.put('/api/projects/unit-locations', d) : axios.post('/api/projects/unit-locations', d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['unit-locations'] }); reset(); },
  });
  const del = useMutation({
    mutationFn: (id: number) => axios.delete('/api/projects/unit-locations', { data: { id } }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['unit-locations'] }),
  });

  return (
    <div>
      <PageHeader title="Unit Location Creation" />
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-4">
        <div className="xl:col-span-2 bg-white border rounded-lg shadow-sm p-4 space-y-3">
          <h3 className="text-xs font-bold text-slate-600 uppercase border-b pb-2">Add Unit Location</h3>
          <form onSubmit={handleSubmit(d => save.mutate(d))} className="space-y-3">
            ...
            <div>
              ...
              <Input {...register('name')} className="mt-1 h-9 text-sm" placeholder="e.g. East Facing, Corner Unit" />
        ...
        <div className="xl:col-span-3 bg-white border rounded-lg shadow-sm overflow-hidden">
          <div className="bg-purple-700 px-3 py-2 text-white text-xs font-bold uppercase">Unit Locations</div>
